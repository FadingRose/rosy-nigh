...
// Impl MethodABI interface

func (me *Method) MethodName() string {
	return me.Name
}
...
func (method Method) IsSame(other Method) bool {
	if method.Name != other.Name {
		return false
	}
	if len(method.Inputs) != len(other.Inputs) {
		return false
	}
	if len(method.Outputs) != len(other.Outputs) {
		return false
	}
...
func (method *Method) OffsetMap() map[string]uint {
	ret := make(map[string]uint)
	for _, input := range method.Inputs {
		ret[input.Name] = uint(input.Type.GetSize())
	}
	return ret
}
