...
func (abi ABI) IsMethodWithArgs(name string) bool {
...
func (abi ABI) IsSame(other ABI) bool {
	if len(abi.Methods) != len(other.Methods) {
		return false
	}
	if len(abi.Events) != len(other.Events) {
		return false
	}
	if len(abi.Errors) != len(other.Errors) {
		return false
	}

	for name, method := range abi.Methods {
		otherMethod, ok := other.Methods[name]
		if !ok || !method.IsSame(otherMethod) {
			return false
		}
	}

	for name, event := range abi.Events {
		otherEvent, ok := other.Events[name]
		if !ok || !event.IsSame(otherEvent) {
			return false
		}
	}

	for name, err := range abi.Errors {
		otherErr, ok := other.Errors[name]
		if !ok || !err.IsSame(otherErr) {
			return false
		}
	}
...
func (abi ABI) ListMethods() []Method {
	var methods []Method
...
func (abi ABI) Format() string {
	methods := abi.ListMethods()
	var res string
	res += fmt.Sprintf("Contract has %d methods\n", len(methods))
	for i, method := range methods {
		res += fmt.Sprintf("%d: %s(%s) %s\n", i, method.Name, method.Inputs.String(), method.Outputs.String())
	}
	events := abi.ListEvents()
	res += fmt.Sprintf("Contract has %d events\n", len(abi.Events))
	for j, event := range events {
		res += fmt.Sprintf("%d: %s(%s)\n", j, event.Name, event.Inputs.String())
	}
	return res
}

func (abi ABI) GetConstructor() Method {
